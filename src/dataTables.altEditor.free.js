...
(function (factory) {
  ...
  $.extend(altEditor.prototype, {
    ...
    _openEditModal: function () {
      ...
      var columns = this.completecolumns();
      ...
      for (var j in columns) {
        if (columns[j].name != null) {
          var jquerySelector =
            "#" + columns[j].name.toString().replace(/\./g, "\\.");

          var arrIndex = columns[j].name.toString().split(".");
          ...
          // Added Select2
          if (columns[j].type.indexOf("select") >= 0 && columns[j].select2) {
          ...
          // Added checkbox
          else if (columns[j].type.indexOf("checkbox") >= 0) {
            if (
          ...
          // Added date
          else if (columns[j].type.indexOf("date") >= 0) {
            if (columns[j].dateFormat !== "") {
              var mDate = moment(this._quoteattr(selectedValue));
              if (mDate && mDate.isValid()) {
                $(selector)
                  .find(jquerySelector)
                  .val(mDate.format(columns[j].dateFormat));
              }
            }
          }
     ...
     * @param columns as returned by completecolumns()
     */
    createDialog: function (
      ...
      for (var j in columns) {
        var title = columns[j].title.replace(/(<([^>]+)>)/gi, "").trim();

        //handle hidden fields
        if (columns[j].type.indexOf("hidden") >= 0) {
          data +=
            "<input type='hidden' " +
            "id='" +
            this._quoteattr(columns[j].name) +
            "' " +
            fillAttrs(columns[j], ["name", "value"]) +
            "></input>";
        } else {
          ...
          if (columns[j].type.indexOf("select") >= 0) {
            var options = "",
              optionsArray = columns[j].options;
            ...
            data +=
              ...
              "placeholder='" +
              this._quoteattr(
                columns[j].placeholder ? columns[j].placeholder : title
              ) +
              "' " +
              "data-special='" +
              this._quoteattr(columns[j].special) +
              "' " +
              "data-unique='" +
              columns[j].unique +
              "' " +
          ...
          // Adding Text Area
          else if (columns[j].type.indexOf("textarea") >= 0) {
            data +=
          ...
          // Adding text-inputs and error labels, but also new HTML5 types (email, color, ...)
            ...
            data +=
              ...
              /* ???? */ (columns[j].type.indexOf("readonly") >= 0
                ? "readonly "
                : "") +
              "id='" +
              this._quoteattr(columns[j].name) +
              "' " +
              "title='" +
              this._quoteattr(columns[j].hoverMsg) +
              "' " +
              "placeholder='" +
              this._quoteattr(
                columns[j].placeholder ? columns[j].placeholder : title
              ...
              this._quoteattr(columns[j].style) +
              "' " +
          ...
          if (
            !columns[j].inline ||
            (+j + 1 < columns.length && !columns[+j + 1].inline)
          ) {
      ...
      for (var j in columns) {
        if (columns[j].select2) {
          // Require select2 plugin
          $(selector)
            .find("select#" + columns[j].name)
            .select2(columns[j].select2);
        } else if (columns[j].datepicker) {
          // Require jquery-ui
          $(selector)
            .find("#" + columns[j].name)
            .datepicker(columns[j].datepicker);
        } else if (columns[j].datetimepicker) {
          // Require datetimepicker plugin
          $(selector)
            .find("#" + columns[j].name)
            .datetimepicker(columns[j].datetimepicker);
        }

        // custom onchange triggers
        if (columns[j].editorOnChange) {
          // $.escapeSelector requires jQuery 3.x
          $(selector)
            .find("#" + $.escapeSelector(columns[j].name))
            .attr("alt-editor-id", this._quoteattr(j));
          $(selector)
            .find("#" + $.escapeSelector(columns[j].name))
            .on("change", function (elm) {
              var f = columns[$(this).attr("alt-editor-id")].editorOnChange;
              f(elm, that);
            });
        }
        ...
        //added select sort
        if (columns[j].type.indexOf("select") >= 0) {
          if (columns[j].optionsSortByLabel) {
            var jquerySelector =
              "#" + columns[j].name.toString().replace(/\./g, "\\.");
